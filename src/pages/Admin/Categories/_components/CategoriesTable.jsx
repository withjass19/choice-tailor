  ...
  return (
    <div className="overflow-x-auto">
      <div className="space-y-3 lg:hidden">
        {loading ? (
          <div className="rounded-xl border bg-white p-4 text-sm text-gray-500">Loading categories...</div>
        ) : categories.length === 0 ? (
          <div className="rounded-xl border bg-white p-4 text-sm text-gray-500">No categories found.</div>
        ) : (
          categories.map((item) => (
            <div key={item.id} className="rounded-xl border bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-[#061735]">{item.name}</h3>
              ...
              <div className="mt-3 space-y-2 text-sm text-gray-600">
                ...
                <div className="flex flex-wrap gap-2">
                  {item.featured ? (
                    ...
                    <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">No</span>
                  )}
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button type="button" onClick={() => onEdit(item)} className="rounded-lg border p-2 hover:bg-gray-100">
                  <Pencil size={16} />
                </button>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button type="button" className="rounded-lg border p-2 hover:bg-gray-100">
                      <MoreVertical size={16} />
                    </button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-52">
                    <DropdownMenuItem onClick={() => onDuplicate(item)}><Copy className="mr-2 h-4 w-4" />Duplicate</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => onToggleVisibility(item)}>{item.show_on_homepage ? <><EyeOff className="mr-2 h-4 w-4" />Hide Category</> : <><Eye className="mr-2 h-4 w-4" />Show Category</>}</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => onArchive(item)}><Archive className="mr-2 h-4 w-4" />Archive</DropdownMenuItem>
                    <DropdownMenuSeparator />
